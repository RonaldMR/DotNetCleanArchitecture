...
namespace CleanApp.Infrastructure.Repositories
{
    public class PostgressUserRepository : IUserRepository
    {
        ...
        public PostgressUserRepository(string connectionString)
        {
        ...
        {
            return new UserEntity(new Guid(reader.GetString(0)), reader.GetString(1), reader.GetString(2), reader.GetString(3), reader.GetString(4), reader.GetDateTime(5));
        }
        ...
        public async Task<UserEntity> Create(UserEntity userEntity)
        {
            await using(var conn = new NpgsqlConnection(_connectionString))
            {
                ...
                await using(var cmd = new NpgsqlCommand("INSERT INTO users (id, firstName, lastName, emailAddress, password) VALUES (@id, @firstName, @lastName, @emailAddress, @password) returning *", conn))
                {
